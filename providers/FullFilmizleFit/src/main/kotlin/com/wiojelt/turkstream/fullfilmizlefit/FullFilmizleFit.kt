...
class FullFilmizleFit : MainAPI() {
    ...
    private fun itemTitle(item: Element) = item.selectFirst(".name a")?.text()?.trim()
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        ...
        if ("".isNotBlank()) {
            val pageSections = document.select("").mapNotNull { container ->
                ...
                val rows = container.select(".movie-box").mapNotNull { it.toResult() }
        ...
        val rows = document.select(".movie-box").mapNotNull { it.toResult() }
    ...
    override suspend fun search(query: String): List<SearchResponse> {
        ...
        return app.get(target).document.select(".movie-box").mapNotNull { it.toResult() }
